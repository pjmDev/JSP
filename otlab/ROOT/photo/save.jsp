...
<%
	...
	String bstype = "";
	if(getPara(multi, "bstype")==null){
		bstype = getPara(request,"bstype");
		}
	else{
		bstype = getPara(multi,"bstype");
		}
	out.print("<input type='hidden' name='bstype' id='bstype' value='"+bstype+"' /> ");
	...
	if(IsFound){
		...
		strSQL += "values ('"+getPara(multi, "txtBstitle")+"', '"+session.getAttribute("usid")+"' , '"+session.getAttribute("usname")+"' ,'"+getPara(multi, "txtBstext")+"', now() , now(), "; 
		strSQL += " '"+getPara(multi, "bsnumber")+"', '"+bstype+"' )";
...
<script>
	f.target='_self';
	f.action='view.jsp?bsseq=<%=bsseq%>&bstype<%=bstype%>';
	f.submit();
</script>
